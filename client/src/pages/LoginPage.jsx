...
<div className="w-full min-h-screen bg-login bg-center bg-cover bg-no-repeat fixed top-0 left-0">
      ...
      <div className="flex justify-start items-center mb-4 pl-20">
        <div className='mt-2 text-left text-white'>
          ...
          <p className="text-lg">enabling easy deadline setting and progress tracking.</p>
        </div>
      </div>

      <div className="w-full max-w-md mr-auto pl-20 mt-8">
        {isSigningUp ? (
          <Registration onSignInClick={handleSignInClick} onSignUpSuccess={handleSignUpSuccess} />
        ) : (
          <SignIn onCloseSignUpSuccess={handleCloseSignUpSuccess} onSignUpSuccess={signUpSuccess} onSignUpClick={handleSignUpClick} />
        )}
      </div>
    </div>
  );
